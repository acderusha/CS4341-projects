...
class TestCharacter(CharacterEntity):
                ...
                if (wrld.wall_at(next[0], next[1])):
                    ...
                    if enemyNearChar:
                        ...
                        for e in self.getExplosion(wrld):
                            if self.isWithinMonsterRange(next, e, 0):
                                inEnemyRange = True
                                break
    ...
    # returns whether or not coordinate is within a n-block radius of the monster
    # PARAM: [world, [int, int], [int, int], int]   wrld: the current world configuration
    #                                               [coord.x, coord.y]: the x and y coordinated the agent is located at
    #                                               [monster.x, monster.y]: the x and y coordinated of the goal / exit
    #                                               radius: the radius to check around the monster
    def isWithinMonsterRange(self, coord, monster, radius):
        xRange = []
        yRange = []
        xRange.append(monster[0])
        yRange.append(monster[1])
        for i in range(1, radius+1):
            xRange.append(monster[0]+i)
            xRange.append(monster[0]-i)
            yRange.append(monster[1]+i)
            yRange.append(monster[1]-i)

        return (coord[0] in xRange and coord[1] in yRange)
